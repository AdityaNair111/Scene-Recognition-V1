...
def get_tiny_images(image_paths):
  ...
  for x in range(N):
    temp=np.reshape(cv2.resize(load_image_gray(image_paths[x]),(d,d)),(1,-1))
    temp-=np.average(temp)
    temp/=(np.sum((temp)**2,axis=None))**0.5
    feats[x,:]=temp
  #############################################################################
  #############################################################################
...
def build_vocabulary(image_paths, vocab_size):
  ...
  #############################################################################
  N=len(image_paths)
  _,descriptors = vlfeat.sift.dsift(load_image_gray(image_paths[0]),step=int(30),size=3,fast=True,float_descriptors=True)
  for x in range(1,N):
    _,descriptor =vlfeat.sift.dsift(load_image_gray(image_paths[x]),step=int(30),size=3,fast=True,float_descriptors=True)
    descriptors=np.append(descriptors,descriptor,axis=0)

  vocab=vlfeat.kmeans.kmeans(descriptors,vocab_size)
  #############################################################################
  #############################################################################
...
def get_bags_of_sifts(image_paths, vocab_filename):
  ...
  for i in range(0,N):
    ...
    frame , descriptor = vlfeat.sift.dsift(image,step=3,size=3,fast=True,float_descriptors=True)
    assignments = vlfeat.kmeans.kmeans_quantize(descriptor, vocab)
    hist , _ = np.histogram(assignments, bins = range(d+1))
    temp_feat=(hist/np.sum(hist)).reshape(1,-1)
    for level in range(1,L+1):
      level_temp=[]
      for x in range(0,2**level):
        tempx=(frame[:,0]<((x+1)*image.shape[0]/2**level)) * (frame[:,0]>((x)*image.shape[0]/2**level))
        for y in range(0,2**level):
          tempy=(frame[:,1]<((y+1)*image.shape[1]/2**level)) * (frame[:,1]>((y)*image.shape[1]/2**level))
          temp=(tempx * tempy).astype(int)
          index=np.nonzero(temp)
          hist , _ = np.histogram(assignments[index], bins = range(d+1))
          level_temp=np.append(level_temp,hist/np.sum(hist))
      temp_feat=np.append(temp_feat,level_temp.reshape(1,-1),axis=1)
    if i==0 :
      feats=np.append(feats.reshape(1,0),temp_feat.reshape(1,-1),axis=1)
    else :
      feats=np.append(feats,temp_feat.reshape(1,-1),axis=0)
...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats,
  ...
  #############################################################################
  k=11
  D=sklearn_pairwise.pairwise_distances(train_image_feats,test_image_feats,metric=metric) 
  index=np.argsort(D,axis=0)
  index=index[0:k,:]
  for x in range(D.shape[1]):
    temp= [train_labels[i] for i in index[:,x]] # The labels are gathered
    hist={}
    for w in temp:
      if w in hist:
        hist[w]+=1
        ...
        hist[w]=1
    p=sorted(hist, key = hist.get, reverse = True) #largest value of the histogram 
...
def svm_classify(train_image_feats, train_labels, test_image_feats,C=2503):
  """
  ...
  #svms = {cat: SVC(gamma='scale',kernel='rbf', random_state=0, tol=1e-3, C=3110, max_iter=10000) for cat in categories}
  svms = {cat: SVC(gamma='scale',kernel='rbf', random_state=0, tol=1e-3, C=C, max_iter=35000) for cat in categories}
  test_labels = []
  ...
  #level0
  i=0
  predictions=np.zeros((len(categories),test_image_feats.shape[0]))
  Bin=200
  for cat in categories:
    y=(np.array(train_labels)==cat).astype(int)

    
    weights=1/4
    for j in range(1):
      svms[cat].fit(train_image_feats[:,int(j*Bin):int((j+1)*Bin)],y)
    predictions[i,:]=weights*(svms[cat].decision_function(test_image_feats[:,int(j*Bin):int((j+1)*Bin)]))
    temp=np.zeros((4,test_image_feats.shape[0]))
    for j in range(1,5):
      svms[cat].fit(train_image_feats[:,int(j*Bin):int((j+1)*Bin)],y)
      temp[j-1,:]=svms[cat].decision_function(test_image_feats[:,int(j*Bin):int((j+1)*Bin)])
    predictions[i,:]+=weights*np.sum(temp,axis=0)/4
    weights=1/2
    temp=np.zeros((16,test_image_feats.shape[0]))
    for j in range(5,21):
      svms[cat].fit(train_image_feats[:,int(j*Bin):int((j+1)*Bin)],y)
      temp[j-5,:]=svms[cat].decision_function(test_image_feats[:,int(j*Bin):int((j+1)*Bin)])
    predictions[i,:]+=weights*np.sum(temp,axis=0)/16

    i+=1

  index=np.argsort(-predictions,axis=0)
  index=index[0,:]
  for x in range(index.shape[0]):
    test_labels.append(categories[index[x]])
  #############################################################################
  #############################################################################

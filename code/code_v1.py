...
def build_vocabulary(image_paths, vocab_size):
  ...
  #############################################################################
  N=len(image_paths)
  _,descriptors = vlfeat.sift.dsift(load_image_gray(image_paths[0]),step=40,size=3,fast=True,float_descriptors=True)
  for x in range(1,N):
  	_,descriptor =vlfeat.sift.dsift(load_image_gray(image_paths[x]),step=40,size=3,fast=True,float_descriptors=True)
...
def get_bags_of_sifts(image_paths, vocab_filename):
  ...
  N=len(image_paths)
  for x in range(0,N):
  	_ , descriptor = vlfeat.sift.dsift(load_image_gray(image_paths[x]),step=3,size=3,fast=True,float_descriptors=True)
  	...
  	if x==0 :
  		...
  		feats=np.append(feats,hist.reshape(1,-1),axis=0)
...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats,
  ...
  for x in range(D.shape[1]):
    temp= [train_labels[i] for i in index[:,x]]
    hist={}
    for w in temp:
      if w in hist:
        hist[w]+=1
        ...
        hist[w]=1
    p=sorted(hist, key = hist.get, reverse = True)
...
def svm_classify(train_image_feats, train_labels, test_image_feats,C=851):
  """
  ...
  #svms = {cat: SVC(gamma='scale',kernel='rbf', random_state=0, tol=1e-3, C=750, max_iter=35000) for cat in categories}
  svms = {cat: LinearSVC(random_state=0, tol=1e-3,loss='hinge', C=C ,max_iter=100000) for cat in categories}
  test_labels = []
  ...
  for cat in categories:
  	...
  	svms[cat].fit(train_image_feats,y)
  	predictions[i,:]=svms[cat].decision_function(test_image_feats)
  	i+=1
